...
def get_file_paths(
    dataset_path_dict: dict[str, Any],
        ...
        for path in paths_list:
            ...
            for item in path_obj.rglob("*"):
                ...
                if item.is_file() and is_image_file(item.name):
                    ...
                    file_entries.append(
                        (dataset_name, pre_screen, category, folder_path, file_name)
                    )
...
def main() -> int:
    ...
    try:
        ...
        dataset_name = (
            next(iter(dataset_path_dict.keys())) if dataset_path_dict else "dataset"
        )
